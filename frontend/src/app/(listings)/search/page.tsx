...
// import { RootState } from "@/redux/store";
// import { setListings, setLoading, setError } from "@/redux/slices/listingsSlice";
// import api from "@/utils/axiosClient";
...
//         page: page.toString(),
//         limit: "15",
//         ...filters, // Spread your filters from Redux
//       });

//       const response = await api.get(`/listing/filter?${params}`);
...
//       );
//     } catch (err: any) {
//       dispatch(setError(err.response?.data?.error || "Failed to fetch listings"));
//     }
//   };
...
//       // Optionally refresh current page to update bookmark status
//       fetchListings(currentPage);
//     } catch (error) {
//       console.error("Bookmark error:", error);
//     }
...
//           Showing {listingsData.length} of {pagination?.totalListings || 0} results
//         </h3>
...
const SearchPage = () => {
  ...
  const fetchListings = async (page: number) => {
    try {
      ...
      // Build query params manually to handle number/null types
      const params = new URLSearchParams();
